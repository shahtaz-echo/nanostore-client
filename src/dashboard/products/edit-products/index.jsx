    ...
    fetch(`http://127.0.0.1:8000/api/products/${product.id}/`, {
      ...
      .then((res) => {
        if (res.ok) {
          ...
          setMessage("Product Updated Faild!");
        }
      })
    ...
    <div>
      ...
      <form onSubmit={handleSubmit} className="mt-10 bg-gray-100 p-8">
        ...
        <div className="mt-10 flex items-center gap-2 justify-end">
          <Link
            to="/admin/product-list"
            className="text-center py-2 border border-black/30 w-40 font-medium text-black/75 hover:bg-gray-300 transition duration-300"
          >
            Discard
          </Link>
          <button
            type="submit"
            className="py-2 border border-black/30 w-40 font-medium text-white bg-black/80 hover:bg-black transition duration-300"
          >
            Update Product
          </button>
        </div>
